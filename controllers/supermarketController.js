        ...
        for (let i = 0; i < 4; i++) {
          for (const object of supermarket.schedules) {
            if (parseInt(object.schedulesInit) === currentDate.getDay()) {
              ...
              let hour = new Date()
              const formatHourInit = new Date(hour.setHours(`${object.hourInit.split(':')[0]}`, '00'))
              const formatHourFinish = new Date(hour.setHours(`${object.hourFinish.split(':')[0]}`, '00'))
              let end = new Date(hour.setHours(`${object.hourInit.split(':')[0]}`, '00'))
